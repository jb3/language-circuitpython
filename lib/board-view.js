...
export class BoardView {
    ...
    textareaListeners() {
        ...
        this.repl.addEventListener("keydown", (event) => {
            ...
            if (event.key in keydownOutput) {
               ...
               (process.platform != "darwin" &&     // control + shift on anything else
                ...
                if (event.key.toLowerCase() == "c") {          // Need 'toLowerCase' because on Windows shift key
                    atom.clipboard.write(this.selectedText()); // is pressed so character is capitalized
        ...
        this.repl.addEventListener("beforeinput", (event) => {
            ...
            this.scrollToBottom(true);
            var replace = event.isComposing ? this.charsOverBytes(this.cursorPosition, this.repl.selectionStart - this.repl.selectionEnd) : 0; // Needed for IME
            if (event.data != null) {
    ...
    moveCursorLeft(count) {
        this.moveCursorTo(this.cursorPosition-this.bytesOverChars(this.cursorPosition,count*-1));
    }
    ...
    writeChar(charBuffer, start, end) {
        ...
        this.line = this.line.substring(0, this.cursorPosition) + char + this.line.substring(this.cursorPosition+this.bytesOverChars(this.cursorPosition,1));
    ...
    bytesOverChars(pos, numberOfChars) {
        ...
        for(;numberOfChars > 0; numberOfChars--) {
            pos += step;
            numBytes++;
            var charCode = this.line.charCodeAt(pos);
    ...
    charsOverBytes(pos, numberOfBytes) {
        var step = numberOfBytes > 0 ? 1 : -1;
        numberOfBytes *= step;
        var numChars = 0;
